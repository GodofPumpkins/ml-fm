...
class SynthVST(SynthBase):
    ...
    def load_plugin(self, plugin_path):
        ...
        """
        try:
            # self.engine = rm.RenderEngine(self.sample_rate, self.block_size, self.block_size)
            self.engine = daw.RenderEngine(sample_rate=self.sample_rate, block_size=self.block_size)
            self.synth = self.engine.make_plugin_processor("my_synth", plugin_path)



            if self.synth.get_name() == "my_synth":
                self.loaded_plugin = True
                self.patch = self.synth.get_patch()
                self.parameters = parse_parameters(self.synth.get_plugin_parameters_description())

                for i in range(len(self.overridden_params)):
                    index, value = self.overridden_params[i]
                    self.synth.set_plugin_parameter(int(index), value)
    ...
    def load_patch(self):
        ...
        self.synth.set_patch(self.patch)
    ...
    def randomize_patch(self):
        ...
        if self.loaded_plugin:
            random_patch = []
            for i in range(len(self.parameters) - len(self.overridden_params)):
                random_patch.append(random.uniform(0, 1))
